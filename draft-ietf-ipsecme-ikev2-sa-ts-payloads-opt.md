...
For the initial Child SA that was negotiated as part of an initial IKE exchange (e.g., IKE_AUTH), at the time of its creation the parameters of PFS and KE method for Child SAs are not negotiated.
Therefore, when rekeying the initial Child SA for the first time, a regular rekey MUST be used for the negotiation of these parameters. Then, the next rekey can use the optimized way.
...
Two peers' PFS policy and KE method configurations MUST be the same, otherwise, the rekey (regardless of regular or optimized way) of the initial Child SA created in the IKE_AUTH exchange would fail. This issue is also discussed in detail in {{-child-pfs}}. However, if the PFS policy and KE method(s) for Child SAs can be negotiated during the creation of the initial Child SA via the mechanism like {{-child-pfs}}, then the first rekey of the initial Child SA can use the optimized way instead of the regular way. In this situation, unless there is a configuration change, all regular rekeys can be replaced by the optimized ones.
