...
abbrev: Optimized Rekey Format of IKE & Child SAs
...
If the responder fails processing the optimized rekey request, e.g., receiving a request a non-allowed PFS proposal, it MUST return an error message (What error type?). After receiving the error response of the optimized rekey, the initiator can retry a regular rekey.
...
Two peers' PFS policy and KE method configurations MUST be the same, otherwise the rekey of the Child SA created in the IKE_AUTH exchange would fail. This issue is also detailed discussed in {{child-pfs}}. If the peers use the mechanism like {{child-pfs}} to negotiate the KE method to be used for the initial Child SA during its creation, the optimized rekey method can be used for its rekey and this KE method is inherited.
